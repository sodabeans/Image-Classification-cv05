...
from model import GoogLeNet, ResNet, AlexNet, SqueezeNet
...
def train(data_dir, model_dir, args):
    ...
    model_1 = GoogLeNet(num_classes = num_classes)
    model_2 = AlexNet(num_classes = num_classes)
    model_3 = ResNet(num_classes = num_classes)
    model = VotingClassifier([('GoogLeNet',model_1),('AlexNet',model_2),('ResNet',model_3)],voting='soft').to(device)
    model = torch.nn.DataParallel(model) # have to check this part
...
if __name__ == '__main__':
    ...
    wandb.init(project="ensemble", reinit=True)
